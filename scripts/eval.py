...
from sklearn.metrics import roc_curve, roc_auc_score, precision_recall_curve, roc_auc_score, f1_score, average_precision_score
...
def pixel_values_in_mask(true_vessels, pre_vessels_decode, mask, train_or, dataset):
    ...
    if len(np.shape(pre_vessels_decode))>2:
            
        if dataset=='HRF-AV':
            mask = mask[0,...]

        true_vessels = true_vessels.transpose((1, 2, 0)).reshape((-1,4))
        pre_vessels_decode = pre_vessels_decode.transpose((1, 2, 0)).reshape((-1,4))
        mask = mask.reshape((-1))

        true_vessels = true_vessels[mask!=0]
        pre_vessels_decode = pre_vessels_decode[mask!=0]
            
        return true_vessels, pre_vessels_decode
...
def eval_net(net, dataset_test, loader, device, mode, train_or):
    ...
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for batch in loader:
            ...
            if mode== 'whole':
                ...
                mask_pred_softmax_cpu_ = F.softmax(mask_pred_softmax_cpu,dim=1)
                _,mask_pred_softmax_cpu_decode = torch.max(mask_pred_softmax_cpu_,1)
                ...
                #artery
                #######################################
                label_cpu_encode = np.eye(4, dtype='uint8')[label_cpu]
                label_cpu_encode = label_cpu_encode.transpose((2, 0, 1))
                
                label_cpu_flatten, mask_pred_softmax_cpu_decode_flatten = pixel_values_in_mask(label_cpu, mask_pred_softmax_cpu_decode, mask_cpu, train_or, dataset_test)
                
                mask_pred_softmax_cpu_numpy = mask_pred_softmax_cpu_.numpy()
                label_cpu_encode_flatten, mask_pred_softmax_cpu_encode_flatten = pixel_values_in_mask(label_cpu_encode, mask_pred_softmax_cpu_numpy, mask_cpu, train_or, dataset_test)
                
                _,auc_roc_a,auc_roc_v,auc_roc_u = roc_auc_score(label_cpu_encode_flatten, mask_pred_softmax_cpu_encode_flatten,multi_class='ovr',average=None)
                _,auc_pr_a,auc_pr_v,auc_pr_u = average_precision_score(label_cpu_encode_flatten, mask_pred_softmax_cpu_encode_flatten,average=None)    
